...
static const uint8_t numBytes = 30;
static uint8_t testData[numBytes] = {0,};
static const uint8_t bytesAtATime = 5;
...
void writtenHandle(const GattWriteCallbackParams *Handler)
{
    ...
    if (Handler->handle == characteristic1.getValueAttribute().getHandle())
    {
        ...
        if(buf[0] == 0x01){
          uint8_t bytesToSend[bytesAtATime] = {0,};
          for(uint8_t index = 0; index<numBytes; index+=bytesAtATime){
            for(uint8_t j = 0; j<bytesAtATime; j++){
              bytesToSend[j] = testData[index+j];
            }
            Serial1.println(index);
            ble.updateCharacteristicValue(characteristic3.getValueAttribute().getHandle(), bytesToSend, bytesAtATime);
            delay(100);
          }
        }
    }
...
void readSensors(){
  testData[dataIdx] = (uint8_t) analogRead(sensor1);
  dataIdx++;
  testData[dataIdx] = (uint8_t) analogRead(sensor2);
  dataIdx++;
  if(dataIdx >= numBytes){
    dataIdx = 0;
  }
...
void setup() {
  ...
  // add service
    ...
    // set device name
    ble.setDeviceName((const uint8_t *)"FitSole");
    ...
    ticker.attach_us(readSensors, 200000); // run every .2 seconds
